...
describe("Publishing post from Admin panel: ", () => {
    ...
    test("Should create and publish a post", async () => {
        const payload = { posts: [{ title: "This is First Post", status: "published" }] };
    ...
    test("Should retrieve the pibslished post by ID", async () => {
        ...
        const getPostById = await apiRequest.get(`/ghost/api/admin/posts/${postId}/`);
    ...
    test("Should retrieve the pibslished post by slug", async () => {
        ...
        const getPostBySlug = await apiRequest.get(`/ghost/api/admin/posts/slug/${slug}/`)
    ...
    test("Should update the pibslished post", async () => {
        ...
        const newPayload = { posts: [{ title: "Updated Post", updated_at:updatedAt }] };
        const updatePost = await apiRequest.put(`/ghost/api/admin/posts/${postId}/`, newPayload)
    ...
    test("Should delete the pibslished post", async () => {
        const deleteByID = await apiRequest.delete(`/ghost/api/admin/posts/${postId}/`)
        expect(deleteByID.status).toEqual(204);

        const getDeletedPost = await apiRequest.get(`/ghost/api/admin/posts/${postId}/`);
